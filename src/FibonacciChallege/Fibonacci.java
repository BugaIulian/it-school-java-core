...
public class Fibonacci {
    public static void main(String[] args) {
        ...
        while (!isFibonacci(number)) {
            System.out.println("Please provide a valid number from Fibonacci Sequence:");
            number = scanner.nextInt();
        }
        int indexOfNumberFibonacci = indexOfProvidedFibonacciNumber(number);
    ...
    private static List<Long> benetFormula(int indexOfNumberFibonacci) {
        /**
         Benet's Formula to calculate the next Fibonacci Number F(n+1) = (φ^(n+1) - (1-φ)^(n+1)) / √5
         Where Fn is the provided Fibonacci number and the index being n.
         */
    ...
    public static boolean isFibonacci(int n) {
        /** Formula to check if a given number is a Fibonacci number:
         * (5*N(to the power of 2) + 4) or (5*N(to the power of 2) -4) is a perfect square.*/
    ...
    public static int indexOfProvidedFibonacciNumber(int n) {
        /**Formula to find the index of a given Fibonacci number:
         *  n = log((13 * √5) + 1/2) / log((1 + √5) / 2) */
        double index = 0;
        if (n >= 3) {
